...
fail:
	if (result) {
		if (ret)
			...
			ret = snprintf(result,size,"%s#%llx:%x?result=%d,reply=%s\n", new_loc->desc.name, new_loc->desc.offset,new_loc->desc.extent, ret, rddma_get_option(&params,"request"));
	}
	...
	if (result) {
		if (xfer)
			ret = snprintf(result,size,"%s.%s#%llx:%x/%s.%s#%llx:%x=%s.%s#%llx:%x?result=%d,reply=%s\n",
				       xfer->desc.xfer.name, xfer->desc.xfer.location,xfer->desc.xfer.offset, xfer->desc.xfer.extent,
				       xfer->desc.dst.name, xfer->desc.dst.location,xfer->desc.dst.offset, xfer->desc.dst.extent,
				       xfer->desc.src.name, xfer->desc.src.location,xfer->desc.src.offset, xfer->desc.src.extent,
...
static int xfer_delete(const char *desc, char *result, int size)
{
	...
	if ( (loc = locate_rddma_location(NULL,&params.xfer) ) ) {
		ret = -EINVAL;
...
static int bind_create(const char *desc, char *result, int size)
{
	...
	if ( (xfer = find_rddma_xfer(&params) ) ) {
		if (!params.xfer.offset)
			params.xfer.offset = xfer->desc.xfer.extent;

		xfer->desc.xfer.extent += params.xfer.extent;
	...
	if ( (xfer = find_rddma_xfer (&params) ) ) {
		...
		if ( xfer->desc.xfer.ops && xfer->desc.xfer.ops->bind_delete ) {
			...
			RDDMA_DEBUG (MY_LIFE_DEBUG, "xx %s xfer %s has no bind_delete support\n", 
					__FUNCTION__, xfer->desc.xfer.name);
		}
		rddma_xfer_put (xfer);
	}
	else {
...
out:		
	if (result) {
		if (bind)
			...
			ret = snprintf(result,size,"%s.%s?result=%d,reply=%s\n", params.xfer.name,params.xfer.location, ret,rddma_get_option(&params.src,"request"));
	}
