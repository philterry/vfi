...
{
	return kobj ? container_of(to_kset(kobj), struct vfi_xfer, kset) : NULL;
}

static inline struct vfi_xfer *vfi_xfer_get(struct vfi_xfer *vfi_xfer)
{
	VFI_DEBUG(MY_LIFE_DEBUG,"%s %p\n",__FUNCTION__,vfi_xfer);
	return to_vfi_xfer(kobject_get(&vfi_xfer->kset.kobj));
}

static inline void vfi_xfer_put(struct vfi_xfer *vfi_xfer)
{
	VFI_DEBUG(MY_LIFE_DEBUG,"%s %p\n",__FUNCTION__,vfi_xfer);
	if (vfi_xfer) kobject_put(&vfi_xfer->kset.kobj);
}
...
extern void vfi_xfer_delete(struct vfi_xfer *, struct vfi_desc_param *);
